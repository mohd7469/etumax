...
const InfiniteProductCarousel = ({
  ...
  const products = useMemo(() => {
    return productIds.map(getProductById).filter(Boolean);
  ...
  return (
    <div className={cn(wrapperClass, paddingClass, 'relative py-1')}>
      <button
        type="button"
        onClick={() => scroll('left')}
        className="hidden lg:flex absolute left-1 top-1/2 -translate-y-1/2 z-20 bg-white border shadow-sm rounded-full w-8 h-8 items-center justify-center hover:bg-gray-100"
      ...
      <button
        ...
        className="hidden lg:flex absolute right-1 top-1/2 -translate-y-1/2 z-20 bg-white border shadow-sm rounded-full w-8 h-8 items-center justify-center hover:bg-gray-100"
